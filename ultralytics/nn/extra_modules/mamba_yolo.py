...
__all__ = ("VSSBlock_YOLO", "SimpleStem", "VisionClueMerge", "XSSBlock", "VSSBlock")
...
class XSSBlock(nn.Module):
    def __init__(
        ...
        # 限制drop_path在[0,1]范围内避免bernoulli_报错
        self.drop_path = DropPath(max(0.0, min(1.0, drop_path)))
...
class VSSBlock(nn.Module):
    ...
    def forward(self, input: torch.Tensor):
        B, C, H, W = input.shape
        x = input.permute(0, 2, 3, 1)  # (B, C, H, W) -> (B, H, W, C)
        x_att = self.self_attention(self.ln_1(x))  # SS2D 操作
        x = x + self.drop_path(x_att)
        x = x.permute(0, 3, 1, 2)  # (B, H, W, C) -> (B, C, H, W)
        assert x.shape == (B, C, H, W), "Shape mismatch in VSSBlock"
        return x
